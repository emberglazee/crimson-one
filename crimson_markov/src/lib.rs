...
pub struct MarkovChain {
    ...
    words_to_ids: HashMap<String, u32>, // keys are lowercase
    ids_to_words: Vec<String>,          // values are lowercase
    cased_word_interner: StringInterner<StringBackend<SymbolUsize>>,
    casing_map: HashMap<u32, HashMap<SymbolUsize, u32>>, // id -> {cased_id -> count}
    rng: Rng,
}

impl MarkovChain {
    fn intern_word_and_update_casing(&mut self, word: &str) -> u32 {
        let lower_word = word.to_lowercase();
        let id = *self.words_to_ids.entry(lower_word.clone()).or_insert_with(|| {
            let new_id = self.ids_to_words.len() as u32;
            self.ids_to_words.push(lower_word);
            new_id
        });

        let cased_id = self.cased_word_interner.get_or_intern(word);
        let case_counts = self.casing_map.entry(id).or_default();
...
pub extern "C" fn create_chain() -> *mut MarkovChain {
    Box::into_raw(Box::new(MarkovChain {
        ...
        words_to_ids: HashMap::new(),
        ids_to_words: Vec::new(),
...
pub extern "C" fn train_on_text(ptr: *mut MarkovChain, text_ptr: *const c_char) {
    ...
    if word_ids.len() >= 2 {
        chain.bigram_starters.push(word_ids[0]);
    ...
    if word_ids.len() >= 3 {
        chain.trigram_starters.push((word_ids[0], word_ids[1]));
...
fn get_preferred_casing(chain: &MarkovChain, id: u32) -> String {
    if let Some(case_map) = chain.casing_map.get(&id) {
        if let Some((cased_id, _)) = case_map.iter().max_by_key(|&(_, count)| count) {
            if let Some(word_str) = chain.cased_word_interner.resolve(*cased_id) {
...
fn generate_bigram(
    ...
    if chain.bigram_chain.is_empty() || chain.bigram_starters.is_empty() {
        return Vec::new();
    }
    ...
    if let Some(words) = seed_words {
        if !words.is_empty() {
            let seed_ids: Vec<u32> = words
                .iter()
                .filter_map(|word| chain.words_to_ids.get(&word.to_lowercase()).cloned())
                .collect();
            if !seed_ids.is_empty() {
                if let Some(last_seed_id) = seed_ids.last() {
                    if chain.bigram_chain.contains_key(last_seed_id) {
    ...
    if !seeded {
        current_word_id = chain.bigram_starters[chain.rng.usize(..chain.bigram_starters.len())];
        result_ids.push(current_word_id);
    }

    let words_to_generate = max_words.saturating_sub(result_ids.len());
    for _ in 0..words_to_generate {
        if let Some(next_word_ids) = chain.bigram_chain.get(&current_word_id) {
            if next_word_ids.is_empty() {
                break;
            }
            current_word_id = next_word_ids[chain.rng.usize(..next_word_ids.len())];
            result_ids.push(current_word_id);
        } else {
            break;
        }
...
fn generate_trigram(
    ...
    if chain.trigram_chain.is_empty() || chain.trigram_starters.is_empty() {
        return Vec::new();
    }
    let mut result_ids = Vec::with_capacity(max_words);
    ...
    if let Some(words) = seed_words {
        if !words.is_empty() {
            ...
            if seed_ids.len() >= 2 {
                ...
                if chain.trigram_chain.contains_key(&key) {
                    result_ids = seed_ids;
                    current_pair = key;
                    seeded = true;
                }
                ...
                let possible_starters: Vec<&(u32, u32)> = chain
                    .trigram_chain
                    .keys()
                ...
                if !possible_starters.is_empty() {
                    current_pair =
                        *possible_starters[chain.rng.usize(..possible_starters.len())];
    ...
    if !seeded {
        current_pair = chain.trigram_starters[chain.rng.usize(..chain.trigram_starters.len())];
        result_ids.push(current_pair.0);
        result_ids.push(current_pair.1);
    }
    ...
    let words_to_generate = max_words.saturating_sub(result_ids.len());
    for _ in 0..words_to_generate {
        if let Some(next_word_ids) = chain.trigram_chain.get(&current_pair) {
            if next_word_ids.is_empty() {
                break;
            }
            let next_word_id = next_word_ids[chain.rng.usize(..next_word_ids.len())];
