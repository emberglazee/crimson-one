...
export function parseAIResponse(responseText: string): ParsedAIResponse {
    ...
    // Create a regex to find the start of the first tool call
    const toolCallPattern = new RegExp(`<(?:${toolNames.join('|')})>`, 's')
...
function parseToolCalls(toolCallsText: string): ParsedToolCall[] {
    ...
    // This regex will match <tool_name>...</tool_name> blocks
