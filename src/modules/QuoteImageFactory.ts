...
// Add type definitions for emoji-related data
type ParsedEmoji = {
    id?: string;
    type: 'emoji' | 'ping';
    full: string;
...
const font = 'Roboto'
let emojiImages: EmojiImage[] = []
let hasAnimatedEmojis = false
let speakerEmojis: ParsedEmoji[] = []
let quoteEmojis: ParsedEmoji[] = []
...
export class QuoteImageFactory {
    ...
    public async createQuoteImage(
        ...
        const measureTextWithEmojis = (text: string, emojis: ParsedEmoji[], startIndex: number) => {
            let width = measureCtx.measureText(text).width
            const relevantEmojis = emojis.filter((e: ParsedEmoji) => 
                e.index >= startIndex && 
                e.index < startIndex + text.length
            )
            for (const emoji of relevantEmojis) {
        ...
        const getAllTextLines = (text: string) => text.split('\n')
        const initialSpeakerTextLines = getAllTextLines(speaker)
        const quoteTextLines = getAllTextLines(quote)
        
        let requiredWidth = padding * 2
        for (const line of [...initialSpeakerTextLines, ...quoteTextLines]) {
            const lineWidth = measureTextWithEmojis(line, [...speakerEmojis, ...quoteEmojis], 0)
            requiredWidth = Math.max(requiredWidth, lineWidth + padding * 2)
        }

        // Set canvas width based on required width, bounded by defaultWidth and maxWidth
        const canvasWidth = Math.max(defaultWidth, Math.min(maxWidth, requiredWidth))
        const contentWidth = canvasWidth - padding * 2

        const measureWordWidth = (word: string, startIndex: number, emojis: ParsedEmoji[]) => {
            let width = measureCtx.measureText(word).width
            const wordEmojis = emojis.filter((e: ParsedEmoji) => 
                e.index >= startIndex && 
        ...
        const renderFrame = async (frameIndex: number) => {
            ...
            if (gradient === 'none') {
                ...
                for (const line of speakerLines) {
                    let x = canvasWidth / 2 - ctx.measureText(line).width / 2
