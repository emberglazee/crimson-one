    ...
    triggers: MessageTriggers[] = [
        {
            pattern: [
                ...
                /peg/gim,
                /mick/gim,
                msg => msg.author.id === '244975212448317440'
            ],
            async action(message) {
                if (chance(10))
                    await message.reply(
                        '<:peg:1341742361004212285><:ging:1341742389257310279>'
                    )
            }
        },
        {
            pattern: [/ronald mcdonald/gim],
            async action(message) {
                await message.reply(
                    getRandomElement([
        ...
        {
            pattern: [/embi/gim, /\bember/gim],
            async action(message) {
                if (message.channelId === '1372567739931037890') return
                if (
                    message.mentions.users.has(EMBI_ID) ||
                    message.author.id === EMBI_ID
                )
                    return
                ...
                await emberglaze.send(
                    `${PING_EMBI} https://discord.com/channels/${message.guildId}/${message.channelId}/${message.id}`
                )
            }
        },
        {
            ...
            async action(message) {
                await message.reply(
                    'https://media.discordapp.net/attachments/311334325402599425/1327830607094485073/youtube__uj7Ztu3Alg_576x742_h264.mp4?ex=67e2189e&is=67e0c71e&hm=492a1e8d4e21cdc2495cc0c83111982cfd9ef07f3b0bf4a5240c622259830473&'
                )
            }
        },
        {
            pattern: [/\bash\b/gim, /ashleigh/gim],
            async action(message) {
                const ashleighId = '231808039282409472'
                if (message.channelId === '1372567739931037890') return
                if (message.guildId !== SOLITARY_CONFINEMENT_GUILD_ID) return
                if (
                    message.mentions.users.has(ashleighId) ||
                    message.author.id === ashleighId
                )
                    return
                if (chance(10))
                    await message.reply(
                ...
                await ashleigh.send(
                    `<@${ashleighId}> https://discord.com/channels/${message.guildId}/${message.channelId}/${message.id}`
                )
            }
        },
        {
            pattern: [
                /fish/gim,
                /\byou know what that means\b/gim,
                /effic/gim // pronounciation pun: 'effish' (efficient => ef_fish_ient)
            ],
            async action(message) {
                await message.react('🐟')
                if (chance(10))
        ...
        {
            pattern: [/\bi miss my wife\b/gim],
            async action(message) {
                await message.reply(
                    'https://tenor.com/view/dance-gecko-gif-21029304'
                )
            }
        },
        {
            pattern: [/\bhungry\b/gim],
            async action(message) {
                await Promise.all([
                    message.react('🐴'),
                    message.reply(
        ...
        {
            pattern: [/\b(i'm|im|i am)\b\s+(.+)/gim],
            async action(message) {
                if (!chance(1)) return

                const moderatable =
                    !!message.member && message.member.moderatable

                const match = message.content.match(
                    /\b(i'm|im|i am)\b\s+(.+)/gim
                )
                if (!match) return

                let name = match[0]
                    .replace(/\b(i'm|im|i am)\b\s+/gim, '')
                    .trim()
        ...
        },
        {
            ...
            async action(message) {
                if (chance(50))
                    await message.reply(
                        'https://cdn.discordapp.com/attachments/1261829304271245463/1413087375105724507/r9rgr1evtph91.png?ex=68baa7ac&is=68b9562c&hm=f58001e1a1b88211e0f7449320883aa6114c406b809d23c84a3f2732c259fb8b&'
                    )
            }
        },
        {
            pattern: [/until then/gim],
            async action(message) {
                await message.reply(
                    getRandomElement([
