    ...
    async execute(ctx) {
        ...
        switch (subcommand) {
            case 'wack':
                ...
                await ctx.editReply('Chat history cleared')
                break
            case 'change_shape':
                const vanity = ctx.getStringOption('vanity')
                const uuid = ctx.getStringOption('uuid')
                if (vanity) {
                ...
                await ctx.editReply(`Shape changed to ${shapesInc.shapeUsername}`)
                break
            case 'duel_mode': {
                const enabled = ctx.getBooleanOption('enabled')
                if (enabled) {
                    const shapeAInput = ctx.getStringOption('shape_a')
                    const shapeBInput = ctx.getStringOption('shape_b')
                    const channelId = ctx.getStringOption('channel_id')
                    if (!shapeAInput || !shapeBInput || !channelId) {
                        await ctx.editReply('You must provide shape_a, shape_b, and channel_id to enable duel mode.')
                        return
                    }
                    ...
                    if (!shapeA || !shapeB) {
                        ...
                        await ctx.editReply('Failed to load one or both shapes. Please check the usernames.')
                        return
                    }
                    await shapesInc.enableDuelMode(shapeA, shapeB, channelId)
                    await ctx.editReply(`Duel mode enabled between ${shapeA} and ${shapeB} in <#${channelId}>.`)
                } else {
                    shapesInc.disableDuelMode()
                    await ctx.editReply('Duel mode disabled.')
                }
                break
            }
            case 'set_cookies': {
                try { await ctx.assertEmbi() } catch { return }
                const attachment = ctx.getAttachmentOption('cookies', true)
                if (!attachment.name.toLowerCase().includes('cookie')) {
                    await ctx.editReply('❌ The file name must contain "cookie".')
                    return
                }
                try {
                    ...
                    await ctx.editReply('✅ Cookies updated from file!')
                } catch (err) {
                    await ctx.editReply('❌ Failed to update cookies: ' + (err instanceof Error ? err.message : String(err)))
                }
                break
            }
        }
